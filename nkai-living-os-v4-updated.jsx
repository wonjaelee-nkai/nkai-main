...
const DNA_INNATE = {
  ...
  dayMaster: { value: 0.75, label: "金 · 분석형", element: "金", yinyang: "음", desc: "정교한 분석력 · 원칙 중심 의사결정", publicName: "본질에너지" },
...
const RESONANCE_MAP = {
  NS_dayMaster: {
    axis: "정보 필터링 (N/S)", innate: "金 · 분석형 본질", kipa: KIPA_SCORES.NS.label,
...
// ─── 6개월 시드 (510건 실데이터 기반) ───
// SEED = Architect의 실제 행동 이력 기반 누적 데이터
// 20년 금융 경력(롯데카드) + 70억 이익 창출 + 법인 설립 + 최근 건강 루틴 전환
// infra/social: 20년 금융업 활동이 누적된 실제 수치
// eating/drinking/nightlife: 금융업 접대·네트워킹 문화 반영 → 최근 급감
const SEED = [
  // 롯데카드 말기~퇴사 (유흥·접대 높음, 건강 낮음)
  { date: "2025-06-30", nScore: 32, post: 0.45, sigma: 1.0, dims: { health: 5, morning: 0, bowel: 30, exercise: 0, social: 65, infra: 55, mind: 40, eating: 60, drinking: 55, nightlife: 30 }, entries: [], label: "롯데카드 퇴사" },
  // 베트남 사업구상 (해외 왕래, 소비 높음)
  { date: "2025-08-15", nScore: 30, post: 0.42, sigma: 1.1, dims: { health: 3, morning: 0, bowel: 25, exercise: 0, social: 60, infra: 50, mind: 35, eating: 65, drinking: 50, nightlife: 35 }, entries: [], label: "베트남 6회" },
  // 법인설립 (9.11) — 사업 인프라 급상승
  { date: "2025-09-24", nScore: 38, post: 0.52, sigma: 0.90, dims: { health: 8, morning: 0, bowel: 28, exercise: 0, social: 55, infra: 68, mind: 45, eating: 50, drinking: 40, nightlife: 20 }, entries: [], label: "법인설립" },
  // 특허출원 (10.24) + 아트브이 피해
  { date: "2025-10-31", nScore: 40, post: 0.55, sigma: 0.85, dims: { health: 5, morning: 0, bowel: 25, exercise: 0, social: 45, infra: 72, mind: 38, eating: 55, drinking: 45, nightlife: 25 }, entries: [], label: "특허4건+피해" },
  // 고소장 접수 (12.1) + 유흥 496K 시기
  { date: "2025-11-30", nScore: 36, post: 0.50, sigma: 0.95, dims: { health: 3, morning: 0, bowel: 22, exercise: 0, social: 40, infra: 65, mind: 32, eating: 55, drinking: 50, nightlife: 30 }, entries: [], label: "유흥 496K" },
  // 전환점: 배달→자취식 전환 시작
  { date: "2025-12-15", nScore: 45, post: 0.62, sigma: 0.60, dims: { health: 20, morning: 0, bowel: 40, exercise: 0, social: 38, infra: 70, mind: 48, eating: 25, drinking: 20, nightlife: 10 }, entries: [], label: "배달 전환" },
  // N-KAI 개발 집중 + 건강 루틴 시작
  { date: "2026-01-07", nScore: 55, post: 0.75, sigma: 0.35, dims: { health: 38, morning: 5, bowel: 60, exercise: 5, social: 35, infra: 74, mind: 58, eating: 10, drinking: 8, nightlife: 0 }, entries: [], label: "비빔밥 루틴" },
  // 모닝루틴 정착 + 쾌변공식
  { date: "2026-02-06", nScore: 68, post: 0.88, sigma: 0.12, dims: { health: 55, morning: 55, bowel: 82, exercise: 18, social: 32, infra: 76, mind: 68, eating: 5, drinking: 0, nightlife: 0 }, entries: [], label: "모닝루틴" },
  // 9일 연속 모닝루틴 + admin v7.0 완성
  { date: "2026-02-16", nScore: 72, post: 0.93, sigma: 0.05, dims: { health: 60, morning: 92, bowel: 90, exercise: 22, social: 30, infra: 78, mind: 72, eating: 3, drinking: 0, nightlife: 0 }, entries: [], label: "9일 연속" },
  // 현재: Living Profile 구축 + 97.3% 복원력 실증
  { date: "2026-02-27", nScore: 74, post: 0.95, sigma: 0.02, dims: { health: 62, morning: 95, bowel: 92, exercise: 24, social: 32, infra: 80, mind: 74, eating: 3, drinking: 2, nightlife: 0 }, entries: [], label: "97.3% 복원" },
];
...
- 본질에너지: 金 분석형 (위기대응력 고밀도 0.857) → 원칙적·분석적 성향. 기존 루틴 강화가 자연스러움
...
function calcInnateScore() {
  ...
  // 金 본질에너지 고밀도 → 위기대응력 높음, 경제감각 선천 부재
  const base = 50;
...
function calcThree(dims) {
  // 3대 지표 = 선천 DNA 기저선 + 행동 데이터 성장분
  // 선천은 바닥(floor), 행동이 쌓이면서 올라가는 구조

  // 선천 기저선 (변하지 않음)
  const dnaEcon = DNA_INNATE.wealth.value * 15;       // 경제감각 DNA: 0.0 → 0
  const dnaExpr = DNA_INNATE.expression.value * 15;    // 표현에너지 DNA: 0.143 → 2.1
  const dnaCrisis = DNA_INNATE.strength.value * 15;    // 위기대응력 DNA: 0.857 → 12.9

  // 행동 데이터 성장분 (이게 핵심 — 후천적 진화)
  const behavEcon = (dims.infra || 0) * 0.30          // 사업 활동 → 경제감각 ↑
                  + (dims.eating || 0) * 0.15          // 외식/소비 → 소비 패턴 반영
                  + (dims.social || 0) * 0.15          // 네트워킹 → 경제적 관계 반영
                  + (dims.health || 0) * 0.10          // 자기관리 → 절제형 소비 반영
                  - (dims.nightlife || 0) * 0.20       // 유흥 → 충동 소비 차감
                  - (dims.drinking || 0) * 0.10;       // 음주 → 판단력 저하 차감

  const behavExpr = (dims.social || 0) * 0.35          // 소셜 → 표현에너지 핵심
                  + (dims.morning || 0) * 0.15         // 모닝루틴 → 자기표현 기반
                  + (dims.mind || 0) * 0.20            // 멘탈 → 내적 표현력
                  + (dims.exercise || 0) * 0.10;       // 운동 → 신체 표현

  const behavCrisis = (dims.morning || 0) * 0.20       // 모닝루틴 → 위기대응 기반
                    + (dims.health || 0) * 0.15        // 건강관리 → 체력 기반
                    + (dims.bowel || 0) * 0.10         // 장건강 → 컨디션 안정
                    + (dims.exercise || 0) * 0.10      // 운동 → 회복력
                    - (dims.drinking || 0) * 0.20      // 음주 → 루틴 파괴
                    - (dims.nightlife || 0) * 0.15;    // 유흥 → 체력 소모

  return {
    economic:   Math.min(99, Math.max(1, Math.round(35 + dnaEcon + behavEcon))),
    expressive: Math.min(99, Math.max(1, Math.round(30 + dnaExpr + behavExpr))),
    crisis:     Math.min(99, Math.max(1, Math.round(35 + dnaCrisis + behavCrisis))),
  };
}
...
function generateAlerts(items, dims, history) {
  ...
  else if (hasStress) a.push({ lv: "WARNING", msg: "스트레스 감지 — 분석형 본질 → 논리적 대응 권장", v: 0.50 });
